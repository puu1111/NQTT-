...
DRESULT disk_read (BYTE pdrv,BYTE *buff,DWORD sector,UINT count)
{
	...
	{
		case SD_CARD:                                //只用了一个SD卡。编号0
			res=SD_ReadDisk(buff,sector,count);	     //读取SD卡扇区数据
			while(res)                               //返回值不为0读出错
			{
				SD_Init();	                         //重新初始化SD卡
				res=SD_ReadDisk(buff,sector,count);  //重新读取SD卡扇区数据
			}
			break;
		default:
			res=1; 
...
DRESULT disk_write (BYTE pdrv,const BYTE *buff,DWORD sector,UINT count)
{
	...
	{ 
		case SD_CARD:                                   //只用了一个SD卡。编号0
			res=SD_WriteDisk((u8*)buff,sector,count);   //写扇区 
			while(res)                                  //返回值不为0写出错
			{
				SD_Init();	                              //重新初始化SD卡
				res=SD_WriteDisk((u8*)buff,sector,count); //重新写	
			}
			break;
		default:
			res=1; 
